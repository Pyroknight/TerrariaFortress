	...
	public class FlamethrowerFlame : TFProjectile
	{
		...
		public override void AI()
        {
			...
			if (!projectile.wet && !WorldGen.SolidOrSlopedTile(point.X, point.Y))
			{
                ...
                {
                    Dust dust1 = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, dustType, projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, alpha, color, MathHelper.Clamp(dustScaleTimer * 0.3f, 0f, 7f))];
                    dust1.noGravity = true;
                    dust1.noLight = true;
                }

				Dust dust2 = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, DustID.FlameBurst, projectile.velocity.X * 0.25f, projectile.velocity.Y * 0.25f, 200, default, 0.5f)];
