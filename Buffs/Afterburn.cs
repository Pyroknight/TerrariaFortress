...
namespace TerrariaFortress.Buffs
{
    ...
    {
        public int burnTimer = 31;
        public float dustRadius = 2f;
        ...
        public override void Update(Player player, ref int buffIndex)
        {
			if (Collision.WetCollision(player.position, player.width, player.height) && !player.lavaWet)
            {
				...
				for (int i = 0; i < 20; i++)
				{
					Dust dust1 = Dust.NewDustDirect(player.position, player.width, player.height, DustID.Smoke, 0f, -3f, 180, new Color(255, 255, 255), 4f);
            ...
            for (int i = 0; i < 3; i++)
			{
				Dust dust1 = Dust.NewDustDirect(player.position + new Vector2(-dustRadius, -dustRadius), (int)(player.width + dustRadius * 2), (int)(player.height + dustRadius * 2), ModContent.DustType<AfterburnFlames>(), player.velocity.X * -0.4f, player.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, Main.rand.NextFloat(0.1f, 1.5f));
				dust1.velocity *= 1.8f;
				dust1.velocity.Y -= 0.5f;
			}
			Dust dust2 = Dust.NewDustDirect(player.position + new Vector2(-dustRadius, -dustRadius), (int)(player.width + dustRadius * 2), (int)(player.height + dustRadius * 2), DustID.Fire, player.velocity.X * 0.4f, player.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, 1f);
			dust2.velocity *= 1.8f;
        ...
        public override void Update(NPC npc, ref int buffIndex)
        {
			if (Collision.WetCollision(npc.position, npc.width, npc.height) && !npc.lavaWet)
			{
				if (npc.HasBuff(ModContent.BuffType<Afterburn>()))
				{
					...
					for (int i = 0; i < 20; i++)
					{
						Dust dust1 = Dust.NewDustDirect(npc.position, npc.width, npc.height, DustID.Smoke, 0f, -3f, 180, new Color(255, 255, 255), 4f);
						dust1.noGravity = true;
						dust1.noLight = true;
					}
				}
			...
			for (int i = 0; i < 3; i++)
			{
				Dust dust1 = Dust.NewDustDirect(npc.position + new Vector2(-dustRadius, -dustRadius), (int)(npc.width + dustRadius * 2), (int)(npc.height + dustRadius * 2), ModContent.DustType<AfterburnFlames>(), npc.velocity.X * -0.4f, npc.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, Main.rand.NextFloat(0.1f, 1.5f));
				dust1.velocity *= 1.8f;
				dust1.velocity.Y -= 0.5f;
			}
			Dust dust2 = Dust.NewDustDirect(npc.position + new Vector2(-dustRadius, -dustRadius), (int)(npc.width + dustRadius * 2), (int)(npc.height + dustRadius * 2), DustID.Fire, npc.velocity.X * 0.4f, npc.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, 1f);
			dust2.velocity *= 1.8f;
			dust2.velocity.Y -= 0.5f;
		}
    }
