    ...
    public class Afterburn : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
			if (Collision.WetCollision(player.position, player.width, player.height) && !player.lavaWet)
            {
				...
				for (int i = 0; i < 10; i++)
				{
					Dust dust1 = Main.dust[Dust.NewDust(player.position, player.width, player.height, DustID.Smoke, 0f, -3f, 200, new Color(255, 255, 255), 4f)];
            ...
            if (++burnTimer > 30)
            {
                ...
                CombatText.NewText(new Rectangle((int)player.position.X, (int)player.position.Y, player.width, player.height), new Color(255, 255, 255), 4, false);
                burnTimer = 0;
            }

            for (int i = 0; i < 3; i++)
			{
				Dust dust1 = Main.dust[Dust.NewDust(player.position - new Vector2(2f, 2f), player.width + 4, player.height + 4, ModContent.DustType<AfterburnFlames>(), player.velocity.X * -0.4f, player.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, (Main.rand.Next(15) + 1) * 0.1f)];
				dust1.velocity *= 1.8f;
				dust1.velocity.Y -= 0.5f;
			}
			Dust dust2 = Main.dust[Dust.NewDust(player.position - new Vector2(2f, 2f), player.width + 4, player.height + 4, DustID.Fire, player.velocity.X * 0.4f, player.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, 1f)];
			dust2.velocity *= 1.8f;
        ...
        public override void Update(NPC npc, ref int buffIndex)
        {
			if (Collision.WetCollision(npc.position, npc.width, npc.height) && !npc.lavaWet)
			{
				if (npc.HasBuff(ModContent.BuffType<Afterburn>()))
				{
					...
					for (int i = 0; i < 10; i++)
					{
						Dust dust1 = Main.dust[Dust.NewDust(npc.position, npc.width, npc.height, DustID.Smoke, 0f, -3f, 200, new Color(255, 255, 255), 4f)];
						dust1.noGravity = true;
						dust1.noLight = true;
					}
				}
            ...
            if (++burnTimer > 30)
            {
				if (!npc.onFire)
				{
					CombatText.NewText(new Rectangle((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height), new Color(255, 255, 255), 4, false);
				}
                burnTimer = 0;
            }
			...
			for (int i = 0; i < 3; i++)
			{
				Dust dust1 = Main.dust[Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, ModContent.DustType<AfterburnFlames>(), npc.velocity.X * -0.4f, npc.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, (Main.rand.Next(15) + 1) * 0.1f)];
				dust1.velocity *= 1.8f;
				dust1.velocity.Y -= 0.5f;
			}
			Dust dust2 = Main.dust[Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, DustID.Fire, npc.velocity.X * 0.4f, npc.velocity.Y + Main.rand.Next(-10, -1) * 0.1f, 100, default, 1f)];
			dust2.velocity *= 1.8f;
			dust2.velocity.Y -= 0.5f;
		}
    }
